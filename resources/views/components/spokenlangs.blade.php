<section id="{{ $category['label']['nav_class'] }}" class="languages">
    <hr class="gray-line">
    ...
    <div class="container mx-auto relative mt-16 mb-16">
        <div class="block sm:block md:flex lg:flex xl:flex mb-4">
            @isset($category['data'])
                @php $a=1 @endphp
                @foreach($category['data'] as $spoken_lang)
                    <div id="lang_{{ $a }}" class="w-auto md:w-1/3 lg:w-1/3 xl:w-1/3 lang-icon-item text-center border-b-2 border-gray-200 py-4 last:border-b-0 lg:border-b-0 xl:border-b-0 ">
                        ...
                        <span class="lang-icon-title uppercase" title="{{ $spoken_lang['translate']['name'] }}">{{ $spoken_lang['key'] }}</span>
                        <span class="lang-icon-description">{{ $spoken_lang['translate']['level'] }}</span>
                    </div>
                    @php $a++ @endphp
                @endforeach
            @endisset
